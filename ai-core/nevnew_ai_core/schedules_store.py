...
This is ai-core's copy of `scripts/nevnew_schedules.py` — Docker COPYs
`nevnew_ai_core/` at build time, so ai-core cannot import a sibling from the
repo's top-level `scripts/` directory at runtime. KEEP THE TWO FILES IN SYNC
(schema + rrule-subset semantics) if you change either.

Both this module and `scripts/scheduler_runner.py` (invoked by system cron,
same pattern as `scripts/briefing.py` — n8n's ExecuteCommand hangs Python in
its task-runner sandbox) point at the SAME on-disk SQLite file via a bind
mount: `./data/schedules` on the host == `/data/schedules` in the ai-core
container (see docker-compose.yml). ai-core's tools (schedule_task /
list_schedules / cancel_schedule, see tools/scheduling.py) only ever
create/list/cancel rows here; `scheduler_runner.py` is the only thing that
actually fires a due schedule and pushes to Telegram.
